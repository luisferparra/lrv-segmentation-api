...
class CreateDimDateTable extends Migration {
	...
	public function up()
	{
		Schema::connection('segmentation')->create('dim_date', function(Blueprint $table)
		{
      $table->increments('id');
			$table->integer('date_full')->index();
			...
			$table->boolean('day_of_month');
			$table->smallInteger('day_of_year');
			$table->char('weekday_weekend', 10);
			$table->boolean('week_of_year');
			$table->char('month_name', 10);
			$table->boolean('month_of_year');
			$table->char('is_last_day_of_month', 1);
			$table->boolean('calendar_quarter');
			$table->smallInteger('calendar_year');
			$table->char('calendar_year_month', 10);
			$table->char('calendar_year_qtr', 10);
			$table->boolean('fiscal_month_of_year');
			$table->boolean('fiscal_quarter');
			$table->integer('fiscal_year');
			$table->char('fiscal_year_month', 10);
			$table->char('fiscal_year_qtr', 10);
		});
                
                DB::connection('segmentation')->unprepared("CREATE PROCEDURE `PopulateDateDimension`(BeginDate DATETIME, EndDate DATETIME)
BEGIN
...
");
            ...
            DB::connection('segmentation')->unprepared("CALL PopulateDateDimension('1900/01/01', '2200/12/31');");
            
            DB::connection('segmentation')->unprepared('DROP PROCEDURE `PopulateDateDimension`');
	...
	{
		Schema::connection('segmentation')->drop('dim_date');
	}
